...
export default (events: Map<string, Function[]>, {enableMessageContent}:any) =>{
  const eventArray: Array<String> = Array.from(events.keys());
  var intentBits: Array<Number> = Array(22).fill(0);

  eventArray.forEach( event =>{
    const intentBitShiftArray: Array<Number> = (intents as { [key: string]: any})[event as string];
    if(!intentBitShiftArray) return
    intentBitShiftArray.forEach(intentBitShift =>{
      intentBits[21 - (intentBitShift as number)] = 1
    });
  });
  if(enableMessageContent)  intentBits[21 - 15] = 1  //Change the bit for MESSAGE_CONTENT
